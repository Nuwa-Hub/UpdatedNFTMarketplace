...
const ConnectWalletButton = () => {
  ...
  const connectWallet = async () => {
    if (window.ethereum) {
      ...
      setdata({
        address,
        Balance: (parseInt(balance, 16) / 1000000000000000000).toPrecision(5),
      });
    ...
    <div>
      ...
      <div className="relative group p-2">
        {walletRedirect ? (
          <a
            target={"_blank"}
            href="https://metamask.app.link/dapp/updated-nft-marketplace.vercel.app/"
            className="bg-indigo-600 text-white px-4 py-2 rounded-md"
          >
            Get Wallet
          </a>
        ...
        {!isCONNECTED ? (
          <div className="absolute w-full invisible group-hover:visible bg-transparent p-4">
            <div>
              ...
              <div className="text-red-500"> Balance : {data.Balance}</div>
            </div>
          </div>
        ) : null}
      </div>
